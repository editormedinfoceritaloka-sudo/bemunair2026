...
func (r *Repository) ListPendingOlderThan(age time.Duration) ([]entities.LetterSubmission, error) {
	var rows []entities.LetterSubmission
	return rows, r.db.Preload("Submitter").Preload("AssignedPJ").Where("status IN ? AND created_at <= ?", []string{entities.StatusPending, entities.StatusInReview}, time.Now().Add(-age)).Order("deadline ASC").Find(&rows).Error
}
